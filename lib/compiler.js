...
module.exports = {
  ...
  compile: function(sources, options, callback) {
    ...
    var totalAssemblyBalanceErrors = 0;
    var assemblyBalanceWarning = false;
    warnings.filter(function(error) {
      if(error.indexOf("Inline assembly block is not balanced") > 0) {
        totalAssemblyBalanceErrors += 1;
      }
    });

    // if all the the errors are only assembly balance warnings
    if (errors.length > 0 && totalAssemblyBalanceErrors != errors.length) {
